...
void RTC_PCF8523::rtcReadReg(uint8_t *buf, uint8_t size, uint8_t address)
{
...
//            buf[1] => &address + 1
//      ..... buf[size-1] => &address + size
void RTC_PCF8523::rtcWriteReg(uint8_t address, uint8_t *buf, uint8_t size)
{
  int addrByte = address;
  Wire.beginTransmission(PCF8523_ADDRESS);
  Wire._I2C_WRITE(addrByte);
...
// and returns data
uint8_t RTC_PCF8523::rtcReadReg(uint8_t address)
{
  uint8_t data;
...
void RTC_PCF8523::rtcWriteReg(uint8_t address, uint8_t data)
{
  rtcWriteReg(address, &data, 1);
}

// Writes the reset bit to Control_1
...
void RTC_PCF8523::setAlarm(uint8_t day_alarm, uint8_t hour_alarm, uint8_t minute_alarm)
{
...
void RTC_PCF8523::setAlarm(uint8_t hour_alarm, uint8_t minute_alarm)
{
  Wire.beginTransmission(PCF8523_ADDRESS);
  Wire._I2C_WRITE(0x0A);
  // Enable Minute
...
void RTC_PCF8523::setAlarm(uint8_t minute_alarm)
{
  Wire.beginTransmission(PCF8523_ADDRESS);
  Wire._I2C_WRITE(0x0A);
  // Enable Minute
...
void RTC_PCF8523::getAlarm(uint8_t *buf)
{
  Wire.beginTransmission(PCF8523_ADDRESS);
  Wire._I2C_WRITE(0x0A);
  Wire.endTransmission();
...
void RTC_PCF8523::enableAlarm(bool enable)
{
  uint8_t tmp;
  ...
  if (enable)
  {
    ...
    ackAlarm();
    // Enable the AIE bit
    tmp |= bit(PCF8523_CONTROL_1_AIE_BIT);
  }
  else
  {
    tmp &= ~bit(PCF8523_CONTROL_1_AIE_BIT); // Disable the AIE bit
  }
  rtcWriteReg(PCF8523_CONTROL_1, tmp);
}

void RTC_PCF8523::ackAlarm(void)
{
  uint8_t tmp;
  tmp = rtcReadReg(PCF8523_CONTROL_2);

  tmp &= ~bit(PCF8523_CONTROL_2_AF_BIT); // Clear the AF bit
